...
def make_sim_env(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_stack_cubes.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
...
class TransferCubeTask(BimanualViperXTask):
    ...
    def get_reward(self, physics):
        ...
        reward = 0
        if touch_right_gripper_L or touch_right_gripper_R: # Contact with red cube
            reward = 1
        if touch_right_gripper_L and touch_right_gripper_R and not touch_table: # red cube grasped + lifted
            reward = 2
        if touch_left_gripper_L or touch_left_gripper_R: # Red cube contacts left gripper
            reward = 3
        if touch_left_gripper_L and touch_left_gripper_R and not touch_table: # successful left-gripper grasp and transfer
            reward = 4
        return reward
